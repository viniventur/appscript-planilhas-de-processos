...
function REGBASE_Alteracoes() {
  ...
  var nproc = spreadsheet.getRange('D3').getValue()
  ...
  var headerreg = spreadsheet.getRange('\'Decretos - Alterações\'!B3:J3'); // VALORES PARA REGISTRO TOTAL 
  var regbios = spreadsheet.getRange('\'BIOS\'!J2:R2')
  var novregdata = spreadsheet.getRange('\'Decretos - Alterações\'!K6');
  var mesanobios = spreadsheet.getRange('\'BIOS\'!T2:U2');
  var mesanoreg = spreadsheet.getRange('\'Decretos - Alterações\'!L6:M6')
  var ultlinha = spreadsheet.getLastRow()
  var sheet = spreadsheet.getSheetByName('Decretos - Alterações')
  var processos = sheet.getRange(6, 4, sheet.getLastRow(), 1).getValues().flat();
  var numproc = sheet.getRange(3, 4).getValue();
...
function REGBASE_demais() {

  var data = Utilities.formatDate(new Date(), "GMT-3", "dd/MM/yyyy HH:mm");
  var spreadsheet = SpreadsheetApp.getActive();
  var headerval = spreadsheet.getRange('B3:F3').getValues() // VALORES PARA REGISTRO ATÉ OBJ - OBRIGATORIO
  var nproc = spreadsheet.getRange('B3').getValue()
  var headerreg = spreadsheet.getRange('\'Demais Processos\'!B3:F3'); // VALORES PARA REGISTRO TOTAL 
  var regbios = spreadsheet.getRange('\'BIOS\'!J6:N6')
  var novregdata = spreadsheet.getRange('\'Demais Processos\'!G6');
  var sheet = spreadsheet.getSheetByName('Demais Processos')
  var processos = sheet.getRange(6, 2, sheet.getLastRow(), 1).getValues().flat();
  var numproc = sheet.getRange(3, 2).getValue();
...
function envio_email() {
  ...
  var ss_base = sheet.getSheetByName("Decretos - Alterações");
...
function atualizarfiltromanual() {
  ...
  if (filtragemSheetNames.indexOf(nomeplanilha) !== -1) {
    ...
    var dadosbase = spreadsheet.getRange('\'Decretos - Alterações\'!B5:T')
  ...
  function atualizarfiltromanual() {
    ...
    if (filtragemSheetNames.indexOf(nomeplanilha) !== -1) {
      var sheet = spreadsheet.getSheetByName(nomeplanilha);
      var header = sheet.getRange('B2:L2');
      var dadosbase = spreadsheet.getRange('\'GERAL\'!B2:L')
      var dadosfiltro = sheet.getRange('B2:L');
      var datacel = sheet.getRange('N1');
      if (header.getFilter() == null) {
        sheet = spreadsheet.getSheetByName(nomeplanilha);
        intev = sheet.getRange(3, 2, sheet.getLastRow(), 11);
        intev.clear({contentsOnly: false, skipFilteredRows: false});
        //intev.clearConditionalFormatRules();
        dadosbase.copyTo(header, SpreadsheetApp.CopyPasteType.PASTE_VALUES, false);
        dadosbase.copyTo(header, SpreadsheetApp.CopyPasteType.PASTE_FORMAT, false);
